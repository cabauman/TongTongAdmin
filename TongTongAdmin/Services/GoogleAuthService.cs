...
namespace TongTongAdmin.Services
{
    public class GoogleAuthService
    {
        ...
        private const string ClientID = "405695276825-h7aia5j46cs5prc3gu03f8ei90vnitga.apps.googleusercontent.com";
        private const string ClientSecret = "lFJDGXLkmEDlJBYpaSzY83gi";
        ...
        private const string RevokeEndpoint = "https://accounts.google.com/o/oauth2/revoke";
        ...
        private static GoogleAuthService client = null;
        public static GoogleAuthService Client
        {
            get
            {
                if (client == null) client = new GoogleAuthService();
                return client;
            }
        }
        ...
        public async Task SignOutAsync()
        {
            ...
            HttpResponseMessage response = await httpClient.PostAsync(RevokeEndpoint, content);
            
            if (response.IsSuccessStatusCode)
            {
                foreach(var item in vault.RetrieveAll().Where((x) => x.Resource == TokenTypes.AccessToken.ToString())) vault.Remove(item);
                Settings.Remove(GoogleTokenTime);
                IsAuthorized = false;
            }
        }
